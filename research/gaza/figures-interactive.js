/* Figure 3 of the Gaza destruction study, rebuilt with Observable Plot + D3.
   Prototype: reads a 7 KB summary exported from the Python pipeline
   (research/gaza/data/fig02.json) and draws five responsive, vector panels.

   Loaded after d3.v7.min.js and plot.v0.6.umd.min.js (both `defer`). */
(() => {
  "use strict";

  const lab = document.getElementById("fig-lab");
  if (!lab) return;

  if (!window.Plot || !window.d3) {
    lab.querySelector("#fig-panels").innerHTML =
      '<p class="fig-fallback">The charting libraries did not load. The static figure is below.</p>';
    return;
  }
  const Plot = window.Plot;
  const d3 = window.d3;

  const panels = document.getElementById("fig-panels");
  const slot = (name) => panels.querySelector(`[data-plot="${name}"]`);
  ...
  /* ---- linear interpolation of a series onto an arbitrary x ------------- */
  ...
  function renderA(width) {
    const p = palette();
    const A = DATA.A;
    const curve = controls.smooth.checked ? "catmull-rom" : "linear";
  ...
  function renderAll() {
    if (!DATA) return;
    renderA(widthOf(document.getElementById("panel-A")));
    renderC(widthOf(document.getElementById("panel-C")));
    renderB(widthOf(document.getElementById("panel-B")));
    renderD(widthOf(document.getElementById("panel-D")));
    renderE(widthOf(document.getElementById("panel-E")));
  }
  ...
  function fail(msg) {
    panels.innerHTML =
      '<p class="fig-fallback">Could not load the figure data (' + msg +
      '). The static figure is below.</p>';
  }

  // Data is embedded via research/gaza/data/fig02.js so the page also works when
  // opened straight from disk (file://). Fall back to fetch if that is missing.
